...
def artifact_stem(strategy: str, backend: str, layout: str, dtype: str, m: int, n: int, k: int, batch: int) -> str:
    return (
        f"{sanitize_artifact_component(strategy)}__{sanitize_artifact_component(backend)}__"
        f"{sanitize_artifact_component(layout)}__{sanitize_artifact_component(dtype)}__M{m}_N{n}_K{k}_B{batch}"
    )
...
def run_frontend_test(
    size: int, batch: int, iters: int, strategy: str, backend: str, *, layout: str, dtype: str, verify_results: bool
) -> Path:
    ...
    stem = artifact_stem(strategy, backend, layout, dtype, size, size, size, batch)
...
def default_output_for_backend(backend: str) -> Path:
    return BENCH_OUT_DIR / f"{backend.replace('-', '_')}_gemm_perf.csv"


def main() -> None:
    parser = argparse.ArgumentParser(description="Run the IR2A backend GEMM benchmark via frontend_test + verify.")
    ...
    parser.add_argument("--backend", default="ir2a-asm", choices=["ir2a-asm", "ir2a-bin"])
    ...
    if args.warmup != 10:
        raise SystemExit("run_ir2a_gemm_bench.py currently relies on verify.cpp's fixed 10-iteration warmup")
    if args.repeats <= 0:
        ...
        rows.append(
            {
                ...
                "status": "ok",
                "note": (
                    f"backend={args.backend};strategy={args.strategy};layout={args.layout};dtype={args.dtype};"
    ...
    output = Path(args.output) if args.output else default_output_for_backend(args.backend)

...
from tpp_pytorch_extension._C import _fused_bert_infer as fused_bert_cpp
import time
...
class BertEmbeddings(BlockedModule):
    ...
    def __init__(self, config):
        ...
        layer_dtype = global_layer_dtype
        layer_use_low_prec = layer_dtype != torch.float32
        # We use float16 for embedding layer even when other layers use bfloat8
        self.layer_dtype = layer_dtype
...
class BertEncoder(BlockedModule):
    ...
    def forward(
        ...
        hidden_states = self.encoder.forward(hidden_states, masks)

        hidden_states = BlockedTensor(
            hidden_states, self.blocked_input_signature, orig_dtype
        )
        if hasattr(hidden_states, "unblocked_tensor"):
            hidden_states = hidden_states.unblocked_tensor()
    ...
    try:
        ...
        try:
            if enable:
                ...
                if use_low_prec:
                    global_layer_dtype = (
                        torch.bfloat8 if use_bf8 == True else torch.float16
                    )
                unpad = use_unpad
            yield
        finally:

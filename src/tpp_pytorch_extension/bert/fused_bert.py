...
class BertSelfAttention(BlockedModule):
    ...
    def forward(
        ...
        if self.use_bf16:
            inputs = [
                i.to(torch.float16) if i.is_floating_point() else i for i in inputs
            ]
...
class BertEmbeddings(BlockedModule):
    ...
    def forward(
        ...
        # embeddings = BlockedTensor(embeddings, self.blocked_embed_signature, torch.float16 if self.use_bf16 else torch.float)
